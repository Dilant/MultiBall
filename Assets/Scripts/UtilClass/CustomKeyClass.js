...
export default class CustomKey {
  ...
  get uiTipText() {
    return `${
    {
      ...
      Spanish:
      "Haz clic izquierdo en la interfaz para cambiar la asignación de teclas.",
      日本語: "左クリックでキー設定を変更します。",
      简体中文: "鼠标左键点击 UI 以更改键位。",
      繁體中文: "滑鼠左鍵點擊 UI 以更改鍵位。"
    }[settings.language]}${

    this.ui.allowMouseRightToggle ?
    {
      English: " Right click the screen to toggle the UI.",
      Spanish:
      " Haz clic derecho en la pantalla para mostrar / ocultar la interfaz de usuario.",
  ...
  isMouseButton() {
    return (
      this.key === "Left" || this.key === "Right" || this.key === "Middle");
  ...
  changeKey(key) {
    ...
    if (this.disableConflictKey) {
      for (const k of allPlayerKeys) {
        ...
        if (playerKey === this.key) inputManager.enablePlayerKey(k);
        if (playerKey === key) inputManager.disablePlayerKey(k);
      }
    }
  ...
  showUITip(duration) {
    levelManager.hideTipDelay(
      levelManager.showTip(this.uiTipText),
      duration
    );
  }
